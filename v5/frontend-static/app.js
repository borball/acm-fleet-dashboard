...
function updateSpokeOperators(spokeDetailId, operators) {
    ...
    if (statCard) {
        statCard.innerHTML = `<div class="spoke-stat-card__label spoke-stat-card__label--blue">Operators</div><div class="spoke-stat-card__value spoke-stat-card__value--lg spoke-stat-card__value--blue">${uniqueOperators.length}</div>`;
    }

    const operatorsSection = element.querySelector('.lazy-operators');
    if (operatorsSection && operators.length > 0) {
        const tableHtml = `
            <table class="data-table data-table--sm">
                ...
                <tbody>
                    ...
                    `).join('')}
                    ${uniqueOperators.length > 10 ? `<tr><td colspan="4" class="data-table__more-row">... and ${uniqueOperators.length - 10} more operators</td></tr>` : ''}
                </tbody>
            </table>
        `;
        operatorsSection.innerHTML = tableHtml;
    } else if (operatorsSection) {
        operatorsSection.innerHTML = '<p class="spoke-detail__no-data">No operators installed</p>';
    }
}
...
function renderSpokeDetails(spoke, hubName) {
    const policyCount = spoke.policiesInfo?.length || 0;
    const compliantPolicies = (spoke.policiesInfo || []).filter(p => p.complianceState === 'Compliant').length;
    const policiesOk = compliantPolicies === policyCount;

    return `
        <div id="spoke-detail-${spoke.name}" class="spoke-detail">
            <div class="tabs">
                <button class="spoke-tabs__item spoke-tabs__item--active" onclick="switchSpokeTab(null, 0, '${spoke.name}')">Overview</button>
                <button class="spoke-tabs__item" onclick="switchSpokeTab(null, 1, '${spoke.name}')">Operators</button>
                <button class="spoke-tabs__item" onclick="switchSpokeTab(null, 2, '${spoke.name}')">Policies (${policyCount})</button>
            </div>
            ...
            <div class="spoke-tab-content spoke-tab-content--active">
                <div class="grid grid--7col">
                    <div class="spoke-stat-card">
                        <div class="spoke-stat-card__label">Type</div>
                        <div class="spoke-stat-card__value">SNO</div>
                    </div>
                    <div class="spoke-stat-card">
                        <div class="spoke-stat-card__label">OpenShift</div>
                        <div class="spoke-stat-card__value">${spoke.clusterInfo.openshiftVersion || 'N/A'}</div>
                    </div>
                    <div class="spoke-stat-card">
                        <div class="spoke-stat-card__label">Kubernetes</div>
                        <div class="spoke-stat-card__value">${spoke.version || 'N/A'}</div>
                    </div>
                    <div class="spoke-stat-card">
                        <div class="spoke-stat-card__label">Platform</div>
                        <div class="spoke-stat-card__value">${spoke.clusterInfo.platform || 'N/A'}</div>
                    </div>
                    <div class="spoke-stat-card">
                        <div class="spoke-stat-card__label">Config</div>
                        <div class="spoke-stat-card__value">${spoke.clusterInfo.region || 'N/A'}</div>
                    </div>
                    ...
                    <div class="spoke-stat-card spoke-stat-card--operators lazy-operators-stat">
                        <div class="spoke-stat-card__label spoke-stat-card__label--blue">Operators</div>
                        <div class="spoke-stat-card__value spoke-stat-card__value--lg spoke-stat-card__value--blue">...</div>
                    </div>
                </div>

                ${(spoke.nodesInfo && spoke.nodesInfo.length > 0) ? `
                <div>
                    <h4 class="spoke-detail__hardware-title">Hardware Inventory</h4>
            ...
            <div class="spoke-tab-content">
                <div class="lazy-operators spoke-detail__lazy" id="spoke-operators-${spoke.name}">
                    Loading...
                </div>
            </div>
                    ...
                    <div class="spoke-policy-filter__row">
                        ...
                        <div class="spoke-policy-filter__radios">
                            <label class="filter-bar__radio-label filter-bar__radio-label--sm">
                                <input type="radio" name="spoke-compliance-filter-${spoke.name}" value="" checked onchange="filterSpokePolicies('${spoke.name}')">All
                            </label>
                            <label class="filter-bar__radio-label filter-bar__radio-label--sm">
                                <input type="radio" name="spoke-compliance-filter-${spoke.name}" value="compliant" onchange="filterSpokePolicies('${spoke.name}')">
                                <span>Compliant</span>
                            </label>
                            <label class="filter-bar__radio-label filter-bar__radio-label--sm">
                                <input type="radio" name="spoke-compliance-filter-${spoke.name}" value="noncompliant" onchange="filterSpokePolicies('${spoke.name}')">
                                <span>NonCompliant</span>
                            </label>
                        </div>
                        <button class="btn btn--secondary btn--xs" onclick="clearSpokePolicySearch('${spoke.name}')">Clear</button>
                    </div>
                    <div id="spoke-policy-count-${spoke.name}" class="filter-bar__count--sm">
                        Showing ${policyCount} ${policyCount !== 1 ? 'policies' : 'policy'}
                    </div>
                </div>

                ${renderSpokePolicyList(spoke.policiesInfo || [], hubName, spoke.name)}
                ` : '<p class="spoke-detail__no-data">No policies</p>'}
            </div>
        </div>
    `;
}
